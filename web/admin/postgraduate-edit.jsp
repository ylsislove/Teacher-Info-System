	...
	<body>
		<div class="layui-anim layui-anim-up" style="width: 95%; margin: 30px auto">
			<form class="layui-form layui-form-pane">
				...
				<div class="layui-form-item">
					<div class="layui-inline">
						...
						<div class="layui-input-inline">
							<input type="text" name="userId" lay-verify="required" value="${map.userId}" autocomplete="off" class="layui-input" disabled>
						</div>
					</div>
					...
					<div class="layui-inline">
						...
						<div class="layui-input-inline">
							<input type="text" name="username" value="${map.username}" autocomplete="off" class="layui-input" disabled>
						</div>
					</div>
				</div>
				...
				<input type="text" id="stuId" name="stuId" value="${map.id}" style="display: none;">
				<table id="POSTGRADUATE_table" class="layui-table">
					...
					<tr id="POSTGRADUATE_tr" style="display: none;">
						...
						<td style="padding: 9px 5px">
							<div class="layui-input-inline">
								<input type="radio" class="yes00" name="isFirstTutor00" value="是" title="是" checked="">
								<input type="radio" class="no00" name="isFirstTutor00" value="否" title="否">
							</div>
						</td>
		...
		<script type="text/javascript">
			...
			$("#academicDate").val("${map.academicDate}");
			<c:forEach items="${stuList }" var="stu">
				add('POSTGRADUATE_table', 'POSTGRADUATE_tr');
				$("#graduationDate"+sum).val("${stu.graduationDate}");
				$("#stuName"+sum).val("${stu.stuName}");
				$("#stuId"+sum).val("${stu.stuId}");
				$("#stuType"+sum).val("${stu.stuType}");
				if ("${stu.isFirstTutor}" === '是') {
					document.getElementsByClassName("yes"+sum)[0].setAttribute("checked", "checked");
				} else {
					document.getElementsByClassName("no"+sum)[0].setAttribute("checked", "checked");
		...
		<script>
			layui.use(['form', 'laydate'], function(){
				...
				form.on('submit(add)', function(data) {
					...
					$.ajax({
						type: "POST",
						url: "${pageContext.request.contextPath }/postgraduateEdit.action",
						data: data.field,
