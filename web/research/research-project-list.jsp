...
<html>
	...
	<body>
		...
		<div class="x-body" style="padding-top: 10px">
			<div class="layui-row">
				<form action="${pageContext.request.contextPath }/researchProjectList.action" method="get" class="layui-form layui-col-md12 x-so" style="margin-bottom: 10px">
			...
			<xblock>
				<c:if test="${role == 'admin'}">
					...
					<button class="layui-btn" onclick="x_admin_show('添加${name}信息','${pageContext.request.contextPath }/main.action?page=research-project-add&type=${type}&name=${name}',800,500)"><i class="layui-icon"></i>添加</button>
					<button class="layui-btn" onclick="x_admin_show('批量添加','${pageContext.request.contextPath }/main.action?page=research-project-upload&type=${type}&name=${name}',800,500)"><i class="layui-icon"></i>批量添加</button>
				</c:if>
			...
			<div class="page">
				<div>
					<a class="first" <c:if test="${page.pageNo==1 }">aria-disabled="true" style="pointer-events: none"</c:if>
					   href="${pageContext.request.contextPath }/researchProjectList.action?mode=${mode}&keyword=${keyword}&type=${type}&pageNo=1&role=${role}">首页</a>
					<a class="prev" <c:if test="${page.pageNo==1 }">aria-disabled="true" style="pointer-events: none"</c:if>
					   href="${pageContext.request.contextPath }/researchProjectList.action?mode=${mode}&keyword=${keyword}&type=${type}&pageNo=${page.pageNo-1 }&role=${role}">上一页</a>
					...
					<a class="next" <c:if test="${page.totalPage==0 || page.pageNo==page.totalPage }">aria-disabled="true" style="pointer-events: none"</c:if>
					   href="${pageContext.request.contextPath }/researchProjectList.action?mode=${mode}&keyword=${keyword}&type=${type}&pageNo=${page.pageNo+1 }&role=${role}">下一页</a>
