...
<%--					<input class="layui-input" placeholder="截止时间" name="end" id="end">--%>
			...
			<xblock>
				<c:if test="${role == 'admin'}">
					...
					<button class="layui-btn" onclick="x_admin_show('添加专利信息','${pageContext.request.contextPath }/main.action?page=patent-add',800,500)"><i class="layui-icon"></i>添加</button>
					<button class="layui-btn" onclick="x_admin_show('批量添加','${pageContext.request.contextPath }/main.action?page=patents-upload',800,500)"><i class="layui-icon"></i>批量添加</button>
				</c:if>
			...
			<table class="layui-table">
				...
				<tbody>
					...
					<tr>
						<td>
							<div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='${patent.id}'><i class="layui-icon">&#xe605;</i></div>
						</td>
						...
						<td>${patent.title}</td>
						<td>
							<button type="button" onclick="x_admin_show('查看专利详情','${pageContext.request.contextPath }/patentDetail.action?id=${patent.id }',1000,550)" class="layui-btn layui-btn-xs layui-btn-normal"><i class="layui-icon">&#xe63c;</i> 详情</button>
						</td>
						<c:if test="${role == 'admin'}">
							<td class="td-manage">
								<a title="编辑" onclick="x_admin_show('编辑专利信息','${pageContext.request.contextPath }/patentEditShow.action?id=${patent.id }',800,500)" href="javascript:;">
		...
		<script>
			function exportE() {
				window.open('${pageContext.request.contextPath }/research/patents-export2.jsp?role=${role}');
			}
		</script>
		...
		<script>
			...
			function member_del(obj, id) {
				layer.confirm('确认要删除吗？', function(index) {
					...
					$.ajax({
						type: "POST",
						url: "${pageContext.request.contextPath }/patentDelete.action",
						data: { id:id},
						success:function(data){
							if (data !== "") {
								...
								layer.alert("已删除", {
									...
									location.replace("${pageContext.request.contextPath }/patentList.action?role=${role}&pageNo="+curPageNo)
								});
			...
			function delAll(argument) {
				...
				layer.confirm('确认要删除吗？', function(index) {
					...
					$.ajax({
						...
						url: "${pageContext.request.contextPath }/patentDeleteAll.action",
						data: { data:data },
						success:function(data){
							if (data !== "") {
								layer.alert(data, {
									...
									//刷新页面
									location.replace("${pageContext.request.contextPath }/patentList.action?role=${role}&pageNo="+${page.pageNo})
								});
							}
							else {
								layer.alert("批量删除成功", {
